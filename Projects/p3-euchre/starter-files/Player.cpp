...
It is absolutely right to create the name as a private member variable and use
the constructor to set it.

So, whatever you're seeing in the hpp file is just the base class.
 You'll need to create two classes One for Human and Other for simple, and
derive the stuff from the Base class.*/
...
class SimplePlayer : public Player {
 ...
 public:
  SimplePlayer(const std::string& name) : player_name(name) { next = 0; }

  const std::string& get_name() const override { return player_name; }
  ...
  bool make_trump(const Card& upcard, bool is_dealer, int round,
    ...
    if (round == 1) {                     // round one
      for (int j = SPADES; j < 4; ++j) {  // goes through all of the suits
        if (upcard.get_suit() == j) {     // if the upcard is equal the suit
          int count = 0;
          for (int i = 0; i < 5; ++i) {  // create a counter at 0 and then loop
                                         // through all the cards
            if ((cards[i].get_suit() == j && cards[i].is_face_or_ace()) ||
                cards[i].is_left_bower(
                    static_cast<Suit>(j))) {  // if the card is the upcard suit
              count++;  // and is a face or ace or left bower it adds to counter
            }
          }
          if (count >= 2) {  // if the suit had more than 2
            order_up_suit =
                static_cast<Suit>(j);  // order the upcard and then return true
            return true;
          }
        }
      }
    ...
    if (round == 2) {
      ...
      int count1 = 0;
      for (int i = 0; i < 5;
           ++i) {  // create a counter at 0 and then loop through all the cards
        if ((cards[i].get_suit() == card_suit && cards[i].is_face_or_ace()) ||
            cards[i].is_left_bower(static_cast<Suit>(card_suit)) ||
            cards[i].is_right_bower(static_cast<Suit>(
                card_suit))) {  // if the card is the upcard suit
          count1++;  // and is a face or ace or left bower it adds to counter
        }
      }
      if (count1 >= 1) {  // if the suit had more than 2
        order_up_suit = static_cast<Suit>(
            card_suit);  // order the upcard and then return true
        return true;
      }
    }
    return false;
  ...
  Card lead_card(Suit trump) override {
    ...
    Card play_card(const Card& led_card, Suit trump) override {
        // When playing a card, Simple Players use a simple strategy that considers
        // only the suit that was led. A more complex strategy would also consider
        // the cards on the table. If a Simple Player can follow suit, they play the
        // highest card that follows suit. Otherwise, they play the lowest card in
        // their hand.
...
class HumanPlayer : public Player {
 ...
 public:
  HumanPlayer(const std::string& name) : player_name(name) { next = 0; }
  ...
  void add_card(const Card& c) override {
    if (next < MAX_HAND_SIZE) {
      cards[next] = c;
      next++;
      std::sort(cards.begin(), cards.begin() + next);
    }
  }

  void print_hand() const {
    for (size_t i = 0; i < static_cast<size_t>(next); ++i)
  ...
  Card lead_card(Suit trump) override {
    ...
    int decisions;
    cin >> decisions;
    Card chosen = cards[decisions];
    cards[decisions] = cards[next - 1];
    next--;
    std::sort(cards.begin(), cards.begin() + next);
    return chosen;
  }

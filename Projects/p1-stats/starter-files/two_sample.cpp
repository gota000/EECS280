...
void print_descriptive_stats(vector<double> data) {
  ...
  cout << "  0th percentile = " << percentile(data, 0.0f) << endl;
  cout << " 25th percentile = " << percentile(data, 0.25f) << endl;
  cout << " 50th percentile = " << percentile(data, 0.5f) << endl;
  cout << " 75th percentile = " << percentile(data, 0.75f) << endl;
  cout << "100th percentile = " << percentile(data, 1.0f) << endl;
}
...
vector<double> mean_diff_sampling_distribution(
  ...
  vector<double> mean_diff_vec;

  for (int i = 0; i < 1000; i++){
    vector<double> a_resample = bootstrap_resample(data_A, i);
    vector<double> b_resample = bootstrap_resample(data_B, i);

    double mean_diff = mean(a_resample) - mean(b_resample);

    mean_diff_vec.push_back(mean_diff);
  }
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  // TODO: Implement this function, removing the assert(false); placeholder.
  double lower_bound = percentile(v, (1.0f - width) / 2);
  double upper_bound = percentile(v, 1.0 - ((1.0f - width) / 2));

  ...
  */
  ...
  public:
    ...
    Value_type& operator[](const Key_type& k) {
      ...
      // could not find and is past the end
      if(it == tree_thingy.end()){
        Pair_type pair(k, Value_type());
        auto val = insert(pair);
    ...
    std::pair<Iterator, bool> insert(const Pair_type& val) {
      if( tree_thingy.find(val) != tree_thingy.end()){
          return {tree_thingy.find(val), false};
      }
    return  {tree_thingy.insert(val), true};
    }

    // EFFECTS : Returns an iterator to the first key-value pair in this Map.
    Iterator begin() const { return tree_thingy.begin(); }

    // EFFECTS : Returns an iterator to "past-the-end".
    Iterator end() const { return tree_thingy.end(); }

  private:
    // Add a BinarySearchTree private member HERE.
    BinarySearchTree<Pair_type, PairComp> tree_thingy;
  };
